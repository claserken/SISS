...
class DeleteTShirt(Task):
    ...
    def run(self):
        ...
        infinite_sampler_deletion = InfiniteSampler(dataset_deletion)
        ...
        dataloader_deletion = torch.utils.data.DataLoader(
            ...
            batch_size=self.cfg.train_batch_size,
            num_workers=self.cfg.dataloader_num_workers,
            sampler=infinite_sampler_deletion
        )
        ...
        def evaluate_model(unet, num_imgs_to_generate=self.cfg.eval_images, batch_size=self.cfg.eval_batch_size, set_generator=False):
            unet.eval()
        ...
        tshirt_img_no_norm = transforms.ToTensor()(Image.open(self.cfg.metrics.classifier.tshirt_path)).to(accelerator.device)
        tshirt_img_with_norm = transform(Image.open(self.cfg.metrics.classifier.tshirt_path)).to(accelerator.device)
        ...
        def log_metrics(unet):
            wandb_batch = {}
            if self.cfg.sampling_steps and global_step % self.cfg.sampling_steps == 0:
                images = evaluate_model(unet, set_generator=False) # fix randomness 
                ...
                if self.cfg.metrics.fraction_deletion:
                    ...
                    frac_deletion, _ = TShirtClassifier.get_tshirt_frequency(images, tshirt_img_no_norm)
                    wandb_batch["metrics/deletion_class_fraction"] = frac_deletion
                    print(f'Fraction tshirt: {frac_deletion}')
                ...
                if self.cfg.metrics.likelihood and (global_step % self.cfg.metrics.likelihood.step_frequency == 0):
                    ...
                    likelihood = likelihood_calculator.evaluate_likelihood(unet, tshirt_img_with_norm.unsqueeze(0))[0][0].item()
            ...
            if self.cfg.metrics.inception_score and (global_step % self.cfg.metrics.inception_score.step_frequency == 0 or deletion_tracker['deletion_steps'] == global_step):
                ...
                # TODO: filter out tshirts and make sure cfg has remove_class disabled
                frac_deletion, tshirt_idxs = TShirtClassifier.get_tshirt_frequency(images, tshirt_img_no_norm)
                print(f'Frac tshirt of inception images: {frac_deletion}')
                inception_calculator.update(images[~tshirt_idxs])
        ...
        accum_loss_x = {}
        accum_loss_a = {}
        while global_step < self.cfg.training_steps:
            unet.train()
            ...
            with accelerator.accumulate(unet):
                ...
                if loss is not None:
                    ...
                    retain_graph = self.cfg.deletion.loss_fn == "importance_sampling_with_mixture" or self.cfg.deletion.loss_fn == "subscore_bernoulli"
